...
namespace Statistics{
	...
	void ComputeAverage(struct ComputedStats<numeric_type> &computedValues, const vector<numeric_type, allocator<numeric_type>>& data){
		numeric_type sum = 0;
	...
	template <typename numeric_type>
	void ComputeMax(struct ComputedStats<numeric_type> &computedValues, const vector<numeric_type, allocator<numeric_type>>& data){
		computedValues.Max = *max_element(data.begin(), data.end());
	}
	
	template <typename numeric_type>
	void ComputeMin(struct ComputedStats<numeric_type> &computedValues, const vector<numeric_type, allocator<numeric_type>>& data){
		computedValues.Min = *min_element(data.begin(), data.end());
	}
	
	template <typename numeric_type>
	struct ComputedStats<numeric_type> ComputeStatistics(struct ComputedStats<numeric_type> &computedValues, const vector<numeric_type,allocator<numeric_type>>& data){
		if(data.size() == 0)
	    	{
