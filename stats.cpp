...
void Statistics::ComputeAverage(struct ComputedStats<numeric_type> &computedValues, const vector<numeric_type>& data){
	numeric_type sum = 0;
...
template <typename numeric_type>
void Statistics::ComputeMax(struct ComputedStats<numeric_type> &computedValues, const vector<numeric_type>& data){
	computedValues.Max = *max_element(data.begin(), data.end());
}

template <typename numeric_type>
void Statistics::ComputeMin(struct ComputedStats<numeric_type> &computedValues, const vector<numeric_type>& data){
	computedValues.Min = *min_element(data.begin(), data.end());
}

template <typename numeric_type>
struct Statistics::ComputedStats<numeric_type> Statistics::ComputeStatistics(struct ComputedStats<numeric_type> &computedValues, const vector<numeric_type>& data){
	if(data.size() == 0)
    {
